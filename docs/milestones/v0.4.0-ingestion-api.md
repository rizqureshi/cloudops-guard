...
customer-controlled CLI uploader — has since been implemented, reviewed,
approved, committed, and pushed** (commit
`760cb03da0909fb47592e3ba18135eb60cb29d34`, `feat: add
customer-controlled report uploader`, following three independent
correction passes covering identifier/timestamp/status-capture
sanitization, endpoint-parsing hardening including hexadecimal IPv4
notation, and a CI-restoration commit
(`4e1c778a3408904af534b8b420b85af193a13074`) fixing one
environment-sensitive `--help`-rendering test). See `CLAUDE.md`'s own
Phase 4E paragraph for the complete description: `cloudops-guard upload
...
underway**, uncommitted, pending independent review: see §I's own Phase
4F entry and `docs/reviews/v0.4.0-phase-4f-security-readiness.md`.
Nothing has been deployed, provisioned, tagged, or released. **Phase
4G-A (Azure implementation and deployment tooling) has since been
implemented, uncommitted, pending independent review** — see §I's own
Phase 4G entry and `docs/deployment/azure-ingestion-production.md`.
Phase 4G-B (real provisioning, deployment, and pilot onboarding) has not
begun.
...
4. **Phase 4E — CLI uploader implementation.**
   **Status: implemented, reviewed, approved, committed, and pushed**
   (commit `760cb03da0909fb47592e3ba18135eb60cb29d34`, plus a CI-
   restoration follow-up at `4e1c778a3408904af534b8b420b85af193a13074`)
   — see `CLAUDE.md`'s own Phase 4E paragraph for the full description,
   including the pre-authorized dependency-boundary relocation, three
   independent correction passes, final test counts, and mutation-
...
5. **Phase 4F — Security review and threat-model validation.**
   **Status: review complete, uncommitted, pending independent review.**
   Produced `docs/reviews/v0.4.0-phase-4f-security-readiness.md`,
   exercising every §G threat against the real implementation (a real
   `uvicorn` loopback server for the concurrency/HTTP-level guarantees,
   never code inspection alone) — no open, unmitigated finding remains;
   two items (production numeric abuse-protection thresholds and a
   distributed backing store for the two limiters) are explicitly
   deferred to Phase 4G, exactly as §F/§G themselves reserve. Also added
   one narrowly-scoped, locally-testable production-hardening guard
   (`src/cloudops_guard/ingestion_api/production_readiness.py`, a
   fail-closed check a future Phase 4G production entrypoint must call
   before serving real traffic — no such entrypoint exists yet) and one
   adversarial regression test closing a previously-untested (but
   already-correct) gap in uploader endpoint validation. Also produced,
   as this phase's own broader scope: a provider-neutral production
   architecture recommendation (`docs/deployment/
   ingestion-production.md`, explicitly labeled "pending explicit human
   approval" — no provider or region is selected), a Phase 4G
   deployment-workflow design (same document, §12 — no executable,
   provider-specific workflow file is added), a preparatory pilot
   runbook using only placeholder values
   (`docs/pilots/ingestion-pilot-runbook.md`), and a Phase 4G
   authorization checklist (`docs/pilots/
   phase-4g-authorization-checklist.md`) naming everything that still
   requires separate, contemporaneous human approval. **Nothing has been
   deployed, provisioned, tagged, or released; no credential or token
   has been issued; no pilot customer has been onboarded.**
...
6. **Phase 4G — Production deployment and pilot authorization, now
   split into two controlled sub-stages.**
   ...
   - *Dependencies*: Phase 4F's review passed with no open, unmitigated
     finding; an explicit, recorded provider/region decision and a
     provider-specific cost estimate/budget approval (below).
   ...
   - *Acceptance gate*: the same explicit, separate, human authorization
     ...
     **An explicit, recorded region/data-residency decision (§H's storage-
     ...
     backups. **This precondition is now satisfied**: Microsoft Azure,
     Canada Central, recorded 2026-09-09 (`docs/pilots/
   ...
   - **Phase 4G-A — Azure implementation and deployment tooling.
     Status: implemented, uncommitted, pending independent review.**
     Real Azure production adapters, a hardened container image, Bicep
     infrastructure-as-code, and a manual-dispatch-only deployment
     workflow — see `docs/deployment/azure-ingestion-production.md` for
     the complete description, cost estimate, and test evidence. **This
     sub-phase creates no Azure resource, contacts no live Azure
     subscription, reads no real credential, and does not dispatch the
     deployment workflow it authors.** No endpoint is live; no token
     exists; no customer data has been uploaded; the static website
     (`web/`) remains a wholly separate, unaffected service.
   - **Phase 4G-B — real provisioning, workflow dispatch, deployment,
     smoke testing, token creation, and pilot onboarding. Status: not
     started.** Requires its own separate, explicit, contemporaneous
     authorization, exactly as every deployment decision this project
     has made since v0.1.0 has — implementing and reviewing Phase 4G-A's
     tooling is not that authorization. See `docs/deployment/
     azure-ingestion-production.md` §9/§11 for its complete provisioning
