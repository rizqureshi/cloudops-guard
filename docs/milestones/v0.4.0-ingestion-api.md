...
**Phase 4C (uncommitted, pending independent review) has since implemented
real Argon2id secret hashing/verification, the `lookup_id`/`secret` token
structure, a framework-independent authentication coordinator, per-scope
authorization, and the three-layer abuse-protection ordering §F
specifies** — see §I for what this covers. **No HTTP API endpoint, no
network-reachable server, no production credential store, no uploader,
and no deployment infrastructure exist yet** — Phase 4C is Python-only
authentication *mechanics*, exhaustively unit-tested, never a live
endpoint or a real customer credential. Phase 4D (the HTTP API) is not
authorized by Phase 4C.

**A subsequent, focused Phase 4C security correction pass** (also
uncommitted) fixed three independently reproduced contract violations in
the initial implementation: token-bearing value objects that resisted
`repr`/`str` inspection but still leaked their secret through
`dataclasses.asdict()`; a public `provision_token(..., hasher=...)`
parameter that let a caller substitute an insecure hasher whose output
would be stored verbatim; and Argon2id verification that did not check
the encoded hash's own algorithm, silently accepting a well-formed
Argon2i or Argon2d hash. See `CLAUDE.md`'s Phase 4C paragraph for the
full description of each fix.
...
No authentication state, tenant information, rate-limit budget, or
