...
**Phase 4D (uncommitted, pending independent review) has since
implemented the four `/api/v1` endpoints (§E) against Phases 4B/4C's
...
test counts. Phase 4E (the CLI uploader) is not authorized by Phase 4D
or any of its three correction passes.
...
3. **Phase 4D — Ingestion API implementation.**
   **Status: implemented (uncommitted, pending independent review)** —
   see the milestone-document status note above and `CLAUDE.md`. Applied
